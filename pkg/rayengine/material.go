...
type material interface {
	scatter(randSource *rand.Rand, in *ray, rec hit) (decision bool, attenuationR, attenuationG, attenuationB float64, pdf float64, scattered *ray)
	scatteringPdf(randSource *rand.Rand, in *ray, rec *hit, scattered *ray) float64
...
func (l *lambertian) scatter(randSource *rand.Rand, in *ray, rec hit) (decision bool, attenuationR, attenuationG, attenuationB float64, pdf float64, scattered *ray) {

	// vec3 target = rec.p + rec.normal + random_in_unit_sphere()
	// scattered = ray(rec.p, unit_vector(target - rec.p), r_in.time())
	// // target - rec.p = rec.p + rec.normal + random_in_unit_sphere() - rec.p = rec.normal + random_in_unit_sphere()
	// alb = albedo->value(rec.u, rec.v, rec.p)
	// pdf = dot(rec.normal, scattered.direction()) / M_PI

	in.origin = rec.p
	in.direction = rec.normal.AddVM(randomInUnitSphere(randSource)).Normalize()
	scattered = in
...
func (l *lambertian) scatteringPdf(randSource *rand.Rand, in *ray, rec *hit, scattered *ray) float64 {
	cosine := rec.normal.Dot(scattered.direction.NormalizeI())
	if cosine < 0 {
		cosine = 0.0
	}
...
func (d *diffuseLight) scatter(randSource *rand.Rand, in *ray, rec hit) (decision bool, attenuationR, attenuationG, attenuationB float64, pdf float64, scattered *ray) {
	return false, 0.0, 0.0, 0.0, 0.0, nil
}

func (d *diffuseLight) scatteringPdf(randSource *rand.Rand, in *ray, rec *hit, scattered *ray) float64 {
	return 0.0
}
